...
static struct pmode_table g_pmode_tbl[DC_MODE_TOTAL] = {
	[DC_MODE_LVC] = { LVC_MODE, ADAPTER_SUPPORT_LVC, 0, false, false, 0, 0, 0, 0, 0, 0 },
	[DC_MODE_SC] = { SC_MODE, ADAPTER_SUPPORT_SC, 0, false, false, 0, 0, 0, 0, 0, 0 },
	[DC_MODE_SC4] = { SC4_MODE, ADAPTER_SUPPORT_SC4, 0, false, false, 0, 0, 0, 0, 0, 0 },
};
...
static void dc_pmode_updata_local_en(struct pmode_table *tbl, bool cc_safe, bool dfg_online)
{
	...
	if ((((tbl->local_mode == LVC_MODE) && (!l_di->cc_protect || cc_safe)) ||
		...
		tbl->local_en = true;
		tbl->base_volt = hw_battery_get_series_num() * BAT_RATED_VOLT *
			dc_comm_get_mode_ratio(tbl->local_mode);
	}
}
...
void dc_pmode_update_mode_power(void)
{
	...
	for (i = 0; i < DC_MODE_TOTAL; i++) {
		...
		dc_update_cable_type(tbl[i].local_mode, ratio);
