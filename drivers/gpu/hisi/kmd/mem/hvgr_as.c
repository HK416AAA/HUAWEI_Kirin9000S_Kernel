...
uint32_t hvgr_cq_sched_retain_ctx(struct hvgr_ctx *ctx)
{
	...
	if (free_as != (uint32_t)HVGR_AS_NR_INVALID) {
		gdev->as_free &= ~((u32)1 << free_as);
		if (free_as != ctx->asid) {
			struct hvgr_ctx *const prev_ctx =
				gdev->as_to_ctx[free_as];

			if (prev_ctx != NULL) {
				hvgr_info(gdev, HVGR_CQ, "%s prev_ctx_%u refcount (%d), free_as=%d\n",
...
void hvgr_free_asid(struct hvgr_ctx * const ctx, uint32_t asid)
{
	...
	ctx->gdev->as_free |= ((u32)1 << ctx->asid);
}
