...
static void hvgr_mem_free_map_area(struct hvgr_ctx * const ctx, struct hvgr_kv_map *map_rec)
{
	...
	while (rbnode != NULL) {
		kvnode = rb_entry(rbnode, struct hvgr_kv_node, node);
		area = container_of(kvnode, struct hvgr_mem_area, node);
		(void)hvgr_mem_unmap_area_nolock(ctx, area);
	...
	uint64_t pages)
{
	...
	if ((area->attribute & HVGR_MEM_MASK_UMD_ACCESS) != 0)
		(void)hvgr_mem_zap_ptes(ctx, area->vma, (area->uva + va_offset),
	...
	uint64_t pages)
{
	...
	if ((area->attribute & HVGR_MEM_MASK_UMD_ACCESS) != 0) {
		if (hvgr_mem_insert_pages(ctx, area->vma, uva_start, page_arr, diff_page) != 0) {
	...
	if (ret != 0) {
		if ((area->attribute & HVGR_MEM_MASK_UMD_ACCESS) != 0)
			(void)hvgr_mem_zap_ptes(ctx, area->vma, uva_start,
				(diff_page << PAGE_SHIFT));
		hvgr_mem_pool_ctx_free(ctx, diff_page, (void **)page_arr, 0,
			hvgr_mem_get_sc_id(area->attribute));
...
static void hvgr_mem_vm_open(struct vm_area_struct *vma)
{
	...
	hvgr_info(gdev, HVGR_MEM, "vma open 0x%lx", vma->vm_start);
}
