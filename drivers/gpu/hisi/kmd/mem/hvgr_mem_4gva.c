	...
	struct hvgr_mem_area * const area)
{
	...
	return true;
map_gva_fail:
	(void)hvgr_mem_zap_ptes(ctx, area->vma, area->uva, (area->max_pages << PAGE_SHIFT));
map_uva_fail:
		...
		struct hvgr_mem_area * const area)
{
	...
	if (unlikely(mem_4gva == NULL)) {
		...
		if ((area->attribute & HVGR_MEM_MASK_UMD_ACCESS) != 0) {
			(void)hvgr_mem_zap_ptes(ctx, area->vma, area->uva,
	...
	hvgr_mem_unmap_gva(area);
	if ((area->attribute & HVGR_MEM_MASK_UMD_ACCESS) != 0) {
		ret = hvgr_mem_zap_ptes(ctx, area->vma, area->uva, (area->max_pages << PAGE_SHIFT));
		hvgr_mem_4gva_destroy_vma(area->vma);
	}
