...
long hvgr_mem_insert_pages(struct hvgr_ctx * const ctx, struct vm_area_struct *vma,
	unsigned long addr, struct page **pages, uint64_t page_nums)
{
	...
	if (!ctx->mem_ctx.ion_flag) {
		if (!hvgr_mem_mm_is_us(&ctx->mem_ctx)) {
			hvgr_err(ctx->gdev, HVGR_MEM,
...
bool hvgr_mem_map_uva(struct hvgr_ctx *ctx, struct hvgr_mem_area * const area)
{
	...
	if (hvgr_mem_insert_pages(ctx, area->vma, area->uva, area->page_array, area->pages) != 0)
		return false;

	return true;
}

long hvgr_mem_zap_ptes(struct hvgr_ctx *ctx, struct vm_area_struct *vma,
...
bool hvgr_mem_mm_is_us(struct hvgr_mem_ctx * const pmem)
{
	bool ret = false;
