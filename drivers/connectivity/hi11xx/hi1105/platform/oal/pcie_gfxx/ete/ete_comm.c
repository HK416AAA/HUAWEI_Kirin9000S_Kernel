...
#define RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM 2
#define RX_HOST_DDR_MODE_ALLOC_SIZE (sizeof(uint32_t) * RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM * ETE_RX_CHANN_MAX_NUMS)
struct pcie_rx_cnt_host_res {
    dma_addr_t dma_handle;
    void *cpu_addr;
};
static struct pcie_rx_cnt_host_res g_rx_cnt_host_res[HCC_CHIP_MAX_DEV] = {
    {(dma_addr_t)NULL, (void *)NULL, },
    {(dma_addr_t)NULL, (void *)NULL, }
};
static uintptr_t g_ete_base_addr_tbl[HCC_CHIP_MAX_DEV] = { 0 };
static const host_ctrl_addr g_hostctrl_addr = {
...
host_ctrl_addr* g_pst_ete_addr = (host_ctrl_addr*)&g_hostctrl_addr;
...
uint64_t pcie_ete_get_tx_sr_base_addr(uintptr_t base_address, uint32_t chan_idx)
{
    ...
    ret = hvalue.bits.tx_ch0_sr_base_addr_h;
    ret <<= 32; // 32 B per
...
uint64_t pcie_ete_get_rx_dr_base_addr(uintptr_t base_address, uint32_t chan_idx)
{
    ...
    ret = hvalue.bits.rx_ch0_dr_base_addr_h;
    ret <<= 32; // 32 B per
...
static uint32_t pcie_ete_rx_cnt_get_devid_by_ete_addr(uintptr_t base_addr)
{
    uint32_t dev_id;
    for (dev_id = HCC_EP_WIFI_DEV; dev_id < HCC_CHIP_MAX_DEV; dev_id++) {
        if (g_ete_base_addr_tbl[dev_id] == base_addr) {
            break;
        }
    }
    return dev_id;
...
uint32_t pcie_ete_rx_get_cnt_from_host_ddr(uint32_t chan, uint32_t pcie_id, uintptr_t base_addr)
{
    uint32_t ret;
    uint32_t *cpu_addr = NULL;
    uint32_t dev_id = pcie_ete_rx_cnt_get_devid_by_ete_addr(base_addr);
    oal_pcie_linux_res *pcie_res = NULL;
    if ((chan >= ETE_RX_CHANN_MAX_NUMS) || (pcie_id >= RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM) ||
        (dev_id >= HCC_CHIP_MAX_DEV)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_get_cnt_from_host_ddr pcie %d chan %d dev_id %d get failed",
                           chan, pcie_id, dev_id);
        return 0;
    }

    pcie_res = oal_pcie_get_linux_res_by_id(dev_id);
    if (pcie_res == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_get_cnt_from_host_ddr get pcie_res fail, dev_id=%d", dev_id);
        return 0;
    }
    mutex_lock(&pcie_res->ep_res.st_rx_cnt_res_lock);
    if (g_rx_cnt_host_res[dev_id].cpu_addr == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_get_cnt_from_host_ddr failed, not init");
        return 0;
    }
    cpu_addr = (uint32_t *)(g_rx_cnt_host_res[dev_id].cpu_addr + pcie_id * ETE_RX_CHANN_MAX_NUMS * sizeof(uint32_t) +
                            chan * sizeof(uint32_t));
    ret = oal_readl(cpu_addr);
    mutex_unlock(&pcie_res->ep_res.st_rx_cnt_res_lock);
    return ret;
}

static void pcie_ete_rx_set_cnt_to_host_ddr(uint32_t chan, uint32_t pcie_id, uint32_t val, uint32_t dev_id)
{
    uint32_t *cpu_addr = NULL;
    oal_pcie_linux_res *pcie_res = NULL;
    if ((chan >= ETE_RX_CHANN_MAX_NUMS) || (pcie_id >= RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM) ||
        (dev_id >= HCC_CHIP_MAX_DEV)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_set_cnt_to_host_ddr pcie %d chan %d  dev_id %dget failed",
                           chan, pcie_id, dev_id);
        return;
    }
    ...
    pcie_res = oal_pcie_get_linux_res_by_id(dev_id);
    if (pcie_res == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_set_cnt_to_host_ddr get pcie_res fail, dev_id=%d", dev_id);
        return;
    }
    mutex_lock(&pcie_res->ep_res.st_rx_cnt_res_lock);
    if (g_rx_cnt_host_res[dev_id].cpu_addr == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_set_cnt_to_host_ddr failed, not init");
        return;
    }
    cpu_addr = (uint32_t *)(g_rx_cnt_host_res[dev_id].cpu_addr + pcie_id * ETE_RX_CHANN_MAX_NUMS * sizeof(uint32_t) +
                            chan * sizeof(uint32_t));
    oal_writel(val, cpu_addr);
    mutex_unlock(&pcie_res->ep_res.st_rx_cnt_res_lock);
}

void pcie_ete_set_rx_cnt_host_addr(uintptr_t base_address, uint32_t chan_idx, uint32_t pcie_id, uint64_t host_addr)
{
    uint32_t addr_l = (uint32_t)(host_addr & 0xFFFFFFFF);
    uint32_t addr_h = (uint32_t)(host_addr >> 32);
    uintptr_t offset_l;
    uintptr_t offset_h;

    /* 目前逻辑只考虑有双PCIE的场景 */
    if (!pcie_id) {
        offset_l = base_address + g_pst_ete_addr->rx_chan_off[chan_idx].rx_ch0_pcie0_host_ddr_addr;
        offset_h = base_address + g_pst_ete_addr->rx_chan_off[chan_idx].rx_ch0_pcie0_host_ddr_addr_h;
    } else {
        offset_l = base_address + g_pst_ete_addr->rx_chan_off[chan_idx].rx_ch0_pcie1_host_ddr_addr;
        offset_h = base_address + g_pst_ete_addr->rx_chan_off[chan_idx].rx_ch0_pcie1_host_ddr_addr_h;
    }
    oal_writel(addr_l, offset_l);
    oal_writel(addr_h, offset_h);
}

int32_t pcie_ete_rx_cnt_host_ddr_mode_enable(uintptr_t base_address, uint32_t enable)
{
    hreg_rx_ch_wr_end_sel value;
    hreg_rx_ch_wr_end_sel read;
    uintptr_t offset = base_address + HOST_CTRL_RB_RX_CH_WR_END_SEL_OFF;
    hreg_get_val(value, offset);
    value.bits.rx_ch_wr_end_sel = !!enable;
    hreg_set_val(value, offset);
    hreg_get_val(read, offset);
    if (read.bits.rx_ch_wr_end_sel != value.bits.rx_ch_wr_end_sel) {
        return -OAL_EFAIL;
    } else {
        return OAL_SUCC;
    }
...
int32_t pcie_ete_rx_cnt_host_ddr_mode_res_init(oal_pcie_linux_res *pcie_res)
{
    ...
    uint32_t dev_id = pcie_res->dev_id;

    if (dev_id >= HCC_CHIP_MAX_DEV) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_cnt_host_ddr_mode_res_init init failed, dev_id %d", dev_id);
        return OAL_FAIL;
    }
    g_ete_base_addr_tbl[dev_id] = (uintptr_t)ep_res->pst_ete_base;
    g_rx_cnt_host_res[dev_id].cpu_addr = dma_alloc_coherent(&pst_pci_dev->dev, RX_HOST_DDR_MODE_ALLOC_SIZE,
                                                            &g_rx_cnt_host_res[dev_id].dma_handle, GFP_KERNEL);
    if (g_rx_cnt_host_res[dev_id].cpu_addr == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_cnt_host_ddr_mode_res_init init failed");
        return -OAL_ENOMEM;
    }
    for (j = 0; j < RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM; j++) {
        for (i = 0; i < ETE_RX_CHANN_MAX_NUMS; i++) {
            ret = oal_pcie_host_slave_address_switch(pcie_res,
                                                     (uint64_t)(g_rx_cnt_host_res[dev_id].dma_handle +
                                                                j * sizeof(uint32_t) * ETE_RX_CHANN_MAX_NUMS +
                                                                i * sizeof(uint32_t)),
                                                     &dst_addr, OAL_TRUE);
            if (ret != OAL_SUCC) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "slave address switch failed ret=%d iova=%llx", ret,
                                   (uint64_t)g_rx_cnt_host_res[dev_id].dma_handle);
                dma_free_coherent(&pst_pci_dev->dev, RX_HOST_DDR_MODE_ALLOC_SIZE, g_rx_cnt_host_res[dev_id].cpu_addr,
                                  g_rx_cnt_host_res[dev_id].dma_handle);
                return ret;
            }
            pcie_ete_set_rx_cnt_host_addr((uintptr_t)ep_res->pst_ete_base, i, j, dst_addr);
            pcie_ete_rx_set_cnt_to_host_ddr(i, j, PCIE_ETE_INVALID_RD_VALUE, dev_id);
        }
    }
    return OAL_SUCC;
}
...
int32_t pcie_ete_rx_cnt_host_ddr_mode_res_release(oal_pcie_linux_res *pcie_res)
{
    ...
    uint32_t dev_id = pcie_res->dev_id;
    if (dev_id >= HCC_CHIP_MAX_DEV) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_ete_rx_cnt_host_ddr_mode_res_release failed, dev_id %d", dev_id);
        return OAL_FAIL;
    }
    mutex_lock(&pcie_res->ep_res.st_rx_cnt_res_lock);
    dma_free_coherent(&pst_pci_dev->dev, RX_HOST_DDR_MODE_ALLOC_SIZE, g_rx_cnt_host_res[dev_id].cpu_addr,
                      g_rx_cnt_host_res[dev_id].dma_handle);
    g_rx_cnt_host_res[dev_id].cpu_addr = (void *)NULL;
    g_rx_cnt_host_res[dev_id].dma_handle = (dma_addr_t)NULL;
    mutex_unlock(&pcie_res->ep_res.st_rx_cnt_res_lock);
    return OAL_SUCC;
}
...
{
    return pcie_ete_rx_get_cnt_from_host_ddr(chan_idx, 0, base_address);
}
