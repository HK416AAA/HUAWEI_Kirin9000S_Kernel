...
OAL_STATIC oal_bool_enum_uint8  hmac_arp_probe_fail_process(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    if (pst_hmac_vap->st_vap_base_info.pwl_enable == OAL_FALSE) {
        pst_hmac_arp_probe->en_arp_probe_on = OAL_FALSE; /* 未使能 PWL 模式，停止arp探测清理type；PWL模式下不清理使能与type */
        pst_hmac_arp_probe->uc_arp_probe_type  = 0;
    }
#else
    pst_hmac_arp_probe->en_arp_probe_on = OAL_FALSE; /* 停止arp探测 */
    pst_hmac_arp_probe->uc_arp_probe_type  = 0;
#endif
...
#ifdef _PRE_WLAN_FEATURE_PWL
    if (uc_arp_probe_type & HMAC_VAP_ARP_PROBE_TYPE_PWL) {
        need_reassoc = OAL_TRUE;
    }
#endif
...
OAL_STATIC uint32_t hmac_arp_probe_timeout(void *p_arg)
{
    ...
    pst_hmac_arp_probe = &(pst_hmac_vap->st_hmac_arp_probe);
    ui_val = (uint32_t)oal_atomic_read(&(pst_hmac_arp_probe->rx_unicast_pkt_to_lan));
    if (ui_val == 0) {
...
void hma_arp_probe_timer_start(oal_netbuf_stru *pst_netbuf, void *p_arg)
{
    ...
    if (pst_hmac_arp_probe->en_init_succ && pst_hmac_arp_probe->en_arp_probe_on == OAL_TRUE &&
        ...
        uc_data_type = mac_get_data_type_from_8023(pst_netbuf, (uint8_t *)pst_mac_ether_hdr,
            MAC_NETBUFF_PAYLOAD_ETH, oal_netbuf_get_len(pst_netbuf), OAM_OTA_FRAME_DIRECTION_TYPE_TX);
        /* 发送方向创建定时器，多次创建定时器 */
